...
import { getServerEnv, loadEnvOnce } from './config'
...
declare global {
    namespace Express {
        ...
        interface Request {
            user?: LoggedInUser
            /**
             * 原始请求体（用于飞书/其他平台 webhook 验签）
             * 仅在 express.json 的 verify 钩子中注入
             */
            rawBody?: Buffer
        }
        namespace Multer {
        ...
        this.app.use(
            express.json({
                ...
                verify: (req: any, _res, buf) => {
                    // 保存原始 body 以支持 webhook 验签（例如飞书 Encrypt Key）
...
export async function start(): Promise<void> {
    loadEnvOnce()
    // Parse & validate env early (keeps defaults, but catches invalid types)
    getServerEnv()
