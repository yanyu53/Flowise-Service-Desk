...
class ChatAnthropic_ChatModels implements INode {
        ...
        this.inputs = [
            {
                ...
                type: 'string',
                description: 'Anthropic 兼容网关地址（例如 DashScope: https://dashscope.aliyuncs.com/apps/anthropic）。不填则读取环境变量 ANTHROPIC_BASE_URL',
                optional: true,
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        const anthropicApiUrlInput = nodeData.inputs?.anthropicApiUrl as string
        ...
        const anthropicApiKey =
            getCredentialParam('anthropicApiKey', credentialData, nodeData) ||
            process.env.ANTHROPIC_AUTH_TOKEN ||
            process.env.ANTHROPIC_API_KEY

        const allowImageUploads = nodeData.inputs?.allowImageUploads as boolean

        const fallbackModelName = process.env.ANTHROPIC_MODEL || 'claude-haiku-4-5'
        const fallbackTemperature = process.env.ANTHROPIC_TEMPERATURE || '0.9'
        const obj: Partial<AnthropicInput> & BaseLLMParams & { anthropicApiKey?: string } = {
            temperature: parseFloat(temperature || fallbackTemperature),
            modelName: modelName || fallbackModelName,
            anthropicApiKey,
            streaming: streaming ?? true
        }

        const anthropicApiUrl = anthropicApiUrlInput || process.env.ANTHROPIC_BASE_URL
        if (anthropicApiUrl) obj.anthropicApiUrl = anthropicApiUrl

        const fallbackMaxTokens = process.env.ANTHROPIC_MAX_TOKENS
        const fallbackTopP = process.env.ANTHROPIC_TOP_P
        if (maxTokens || fallbackMaxTokens) obj.maxTokens = parseInt(maxTokens || fallbackMaxTokens || '0', 10)
        if (topP || fallbackTopP) obj.topP = parseFloat(topP || fallbackTopP || '0')
