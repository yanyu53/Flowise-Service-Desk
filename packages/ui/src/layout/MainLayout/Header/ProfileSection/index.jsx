...
const ProfileSection = ({ handleLogout }) => {
    ...
    const onExport = (data) => {
        const body = {}
        if (data.includes('智能体流')) body.agentflow = true
        if (data.includes('智能体流 V2')) body.agentflowv2 = true
        if (data.includes('助手（自定义）')) body.assistantCustom = true
        if (data.includes('助手（OpenAI）')) body.assistantOpenAI = true
        if (data.includes('助手（Azure）')) body.assistantAzure = true
        if (data.includes('聊天流')) body.chatflow = true
        if (data.includes('聊天消息')) body.chat_message = true
        if (data.includes('聊天反馈')) body.chat_feedback = true
        if (data.includes('自定义模板')) body.custom_template = true
        if (data.includes('文档库')) body.document_store = true
        if (data.includes('执行记录')) body.execution = true
        if (data.includes('工具')) body.tool = true
        if (data.includes('变量')) body.variable = true
                                        ...
                                        <Box sx={{ p: 2 }}>
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <ListItemText primary={<Typography variant='body2'>版本信息</Typography>} />
                                                </ListItemButton>
