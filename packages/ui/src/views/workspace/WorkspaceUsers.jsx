...
const WorkspaceDetails = () => {
    ...
    const unlinkUser = async () => {
        ...
        const confirmPayload = {
            title: '移除用户',
            description: `确认将以下用户从工作区移除吗？\n${userList}`,
            confirmButtonName: '移除',
            cancelButtonName: '取消'
        }
        ...
        if (isConfirmed) {
            try {
                ...
                enqueueSnackbar({
                    message: `已从工作区移除 ${usersSelected.length} 位用户。`,
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
