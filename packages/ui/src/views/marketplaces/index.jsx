...
// 注意：badge/type 用于筛选匹配后端枚举值，显示文本在渲染时再转换为中文
...
// 市场内置模板中文映射（仅影响前端显示，不改变后端数据）
const marketplaceTemplateZhMap = {
    'Agentic RAG': {
        title: '智能体式 RAG',
        description: '基于智能体流 V2 的 RAG 方案，用于对文档进行更强的纠错与问答。'
    },
    'Agents Handoff': {
        title: '智能体交接',
        description: '一个客服智能体，可根据场景把任务交接给不同的智能体处理。'
    },
    'Deep Research With Multi-turn Conversations': {
        title: '多轮对话深度研究',
        description: '通过多轮对话执行网页搜索与综合分析，并生成结构化的研究结论。'
    },
    'Deep Research With Subagents': {
        title: '子智能体协同深度研究',
        description: '将复杂问题拆分分配给多个子智能体执行，再汇总为详细报告。'
    },
    'Human In The Loop': {
        title: '人工介入（HITL）',
        description: '在关键节点引入人工确认/反馈，让流程可控地继续或修正。'
    },
    'Interacting With API': {
        title: '调用 API 交互',
        description: '展示智能体调用外部 API 的常见模式与最佳实践。'
    },
    Iterations: {
        title: '迭代循环',
        description: '对一组条目进行迭代处理，并对每一项执行相同动作。'
    },
    'Simple RAG': {
        title: '基础 RAG',
        description: '最常见的 RAG 模板：检索文档分块并进行问答。'
    },
    'SQL Agent': {
        title: 'SQL 智能体',
        description: '在数据库上进行问题解析、SQL 生成与查询结果回答。'
    },
    'Structured Output': {
        title: '结构化输出',
        description: '让模型以 JSON 等结构化格式输出，便于后续系统消费与校验。'
    },
    'Supervisor Worker': {
        title: '主管-执行者',
        description: '由主管智能体制定计划并调度多个执行者完成任务。'
    },
    Translator: {
        title: '翻译器',
        description: '在不同语言之间进行高质量翻译，可用于多语客服/内容本地化。'
    }
}

const getMarketplaceCardText = (tpl) => {
    const key = tpl?.templateName || tpl?.name || ''
    const mapped = marketplaceTemplateZhMap[key]
    const rawTitle = tpl?.templateName || tpl?.name
    const rawDescription = tpl?.description
    if (!mapped) {
        const translateHeuristic = (text) => {
            ...
            const replacements = [
                ...
                ['With Subagents', '（子智能体协同）'],
                ['Human In The Loop', '人工介入'],
                ['Structured Output', '结构化输出'],
                ['Supervisor Worker', '主管-执行者'],
                ['Query Engine', '查询引擎'],
                ['SubQuestion', '子问题'],
                ...
                ['Input Moderation', '输入安全审核'],
                ['Prompt Chaining', '提示词链式编排'],
                ['Chain', '链'],
                ['Agent', '智能体'],
                ['Assistant', '助手'],
                ['Iterations', '迭代循环'],
        ...
        return {
            title: translateHeuristic(rawTitle),
            description: translateHeuristic(rawDescription)
        }
    }
    return {
...
const Marketplace = () => {
    ...
    const onDeleteCustomTemplate = async (template) => {
        const confirmPayload = {
            title: `删除`,
            description: `确认删除自定义模板 ${template.name} 吗？`,
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        <Select
                                            ...
                                            renderValue={(selected) => selected.map((v) => badgeLabelMap[v] ?? v).join(', ')}
                                            MenuProps={MenuProps}
                                        ...
                                        >
                                            {badges.map((name) => (
                                                ...
                                                >
                                                    ...
                                                    <ListItemText primary={badgeLabelMap[name] ?? name} />
                                                </MenuItem>
                                            ))}
                                        </Select>
                                    </FormControl>
                                    ...
                                    >
                                        ...
                                        <Select
                                            ...
                                            renderValue={(selected) => selected.map((v) => typeLabelMap[v] ?? v).join(', ')}
                                            MenuProps={MenuProps}
                                            sx={getSelectStyles(theme.palette.grey[900] + 25, theme?.customization?.isDarkMode)}
                                        >
                                            {types.map((name) => (
                                                ...
                                                >
                                                    ...
                                                    <ListItemText primary={typeLabelMap[name] ?? name} />
                                                </MenuItem>
                                            ))}
                                        </Select>
                                    </FormControl>
                        ...
                        {hasPermission('templates:marketplace') && hasPermission('templates:custom') && (
                            <Stack direction='row' justifyContent='space-between' sx={{ mb: 2 }}>
                                ...
                                <Autocomplete
                                    ...
                                    renderInput={(params) => <TextField {...params} label='使用场景' />}
                                    sx={{
                                        width: 300
                                    }}
                                    limitTags={2}
                                                                ...
                                                                >
                                                                    {(data.type === 'Chatflow' ||
                                                                        ...
                                                                        <ItemCard
                                                                            ...
                                                                            data={{
                                                                                ...data,
                                                                                templateName: getMarketplaceCardText(data).title,
                                                                                description: getMarketplaceCardText(data).description
                                                                            }}
