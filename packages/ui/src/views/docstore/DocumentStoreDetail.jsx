...
const DocumentStoreDetails = () => {
    ...
    const onLoaderDelete = (file, vectorStoreConfig, recordManagerConfig) => {
        ...
        const loaderName = file.loaderName || '未知'
        let sourceName = ''
        ...
        let description = `确认删除 "${displayName}" 吗？这将删除文档库中与其关联的全部分块。`
        ...
        if (
            ...
            description = `确认删除 "${displayName}" 吗？这将删除文档库中与其关联的全部分块，并移除向量数据库中的实际数据。`
        }
    ...
    const onStoreDelete = (vectorStoreConfig, recordManagerConfig) => {
        let description = `确认删除文档库 ${getSpecificDocumentStore.data?.name} 吗？这将删除文档库中所有关联加载器与分块。`
        ...
        if (
            ...
            description = `确认删除文档库 ${getSpecificDocumentStore.data?.name} 吗？这将删除文档库中所有关联加载器与分块，并移除向量数据库中的实际数据。`
        }
    ...
    const onStoreRefresh = async (storeId) => {
        const confirmPayload = {
            title: '刷新全部加载器并重新写入所有分块？',
            description: '该操作会重新处理所有加载器并写入全部分块，可能需要一些时间。',
                                ...
                                <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                                    ...
                                    >
                                        <TableRow>
                                            ...
                                            <StyledTableCell>分块数</StyledTableCell>
