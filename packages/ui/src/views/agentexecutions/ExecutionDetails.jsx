...
export const ExecutionDetails = ({ open, isPublic, execution, metadata, onClose, onProceedSuccess, onUpdateSharing, onRefresh }) => {
    ...
    const buildTreeData = (nodes) => {
        ...
        iterationGroups.forEach((iterationMap, parentId) => {
            iterationMap.forEach((nodeIds, iterationIndex) => {
                ...
                const iterationLabel = `迭代 #${iterationIndex}`
    ...
    const onSharePublicly = () => {
        ...
        updateExecutionApi.request(localMetadata.id, { isPublic: newIsPublic }).then(() => {
            ...
            dispatch(
                enqueueSnackbarAction({
                    message: newIsPublic ? '执行记录已公开分享' : '执行记录已取消公开',
                    options: {
                        key: new Date().getTime() + Math.random(),
                        variant: 'success',
                        action: (key) => (
        ...
        <Box sx={{ display: 'flex', height: '100%', flexDirection: 'row' }}>
                    ...
                    <Box>
                        {!isPublic && (
                            <Chip
                                ...
                                variant='outlined'
                                label={localMetadata?.agentflow?.name || localMetadata?.agentflow?.id || '前往智能体流'}
                                className={'button'}
                        ...
                        {!isPublic && !localMetadata.isPublic && (
                            <Chip
                                ...
                                variant='outlined'
                                label={updateExecutionApi.loading ? '更新中...' : '分享'}
                                className={'button'}
                        ...
                        {!isPublic && localMetadata.isPublic && (
                            <Chip
                                ...
                                variant='outlined'
                                label={updateExecutionApi.loading ? '更新中...' : '公开'}
                                className={'button'}
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', alignContent: 'center' }}>
                            <Typography sx={{ flex: 1, mt: 1 }} color='text.primary'>
                                {metadata?.updatedDate ? moment(metadata.updatedDate).format('MMM D, YYYY h:mm A') : '无'}
                            </Typography>
                            <IconButton
            ...
            >
                {selectedItem && selectedItem.data ? (
                    ...
                    <Typography color='text.secondary'>该项目暂无可用数据</Typography>
                )}
            </Box>
        </Box>
    )
