    ...
    const onSubmitResponse = async (type, feedback = '') => {
        ...
        try {
            ...
            if (response && response.data) {
                enqueueSnackbar('提交响应成功', { variant: 'success' })
                if (onProceedSuccess) onProceedSuccess(response.data)
            }
        } catch (error) {
            console.error(error)
            enqueueSnackbar(error?.message || '提交响应失败', { variant: 'error' })
        } finally {
            setIsLoading(false)
            setLoadingMessage('')
        }
            ...
            if (condition.type === 'string' && condition.operation === 'equal' && condition.value1 === '' && condition.value2 === '') {
                return (
                    ...
                    >
                        ...
                        <Typography variant='body1'>Else 条件已满足</Typography>
                            <Chip
            ...
            return (
                ...
                >
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        <Typography variant='subtitle2'>条件 {index}</Typography>
                        <Chip
                            label={condition.isFulfilled ? '已满足' : '未满足'}
                            size='small'
                            variant='filled'
                    ...
                    {data.output && data.output.availableTools && data.output.availableTools.length > 0 && (
                        <Box>
                            ...
                            {data.output.availableTools.length > 5 && (
                                ...
                                >
                                    {showAllTools ? '收起' : `再显示 ${data.output.availableTools.length - 5} 个`}
                                </Button>
                            )}
                        </Box>
                    )}
                                        ...
                                        {message.additional_kwargs.usedTools.map((tool, index) => {
                                            return tool ? (
                                                <Chip
                                                    ...
                                                    onClick={() => onUsedToolClick(tool, '已使用工具')}
                                                />
                                            ) : null
                                        })}
                                    </div>
            ...
            {status === 'STOPPED' && (
                <>
                    ...
                    <Dialog maxWidth='md' fullWidth open={openFeedbackDialog} onClose={() => !isLoading && setOpenFeedbackDialog(false)}>
                        <DialogTitle variant='h5'>填写反馈</DialogTitle>
                        <DialogContent>
                            <TextField
