...
const Canvas = () => {
    ...
    const canvasTitle = URLpath.includes('agentcanvas') ? '智能体流' : '对话流'
    ...
    const handleDeleteFlow = async () => {
        const confirmPayload = {
            title: '删除',
            description: `确认删除${canvasTitle} ${chatflow.name}吗？`,
    ...
    useEffect(() => {
        if (getSpecificChatflowApi.data) {
            ...
            errorFailed(`获取${canvasTitle}失败：${getSpecificChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        if (createNewChatflowApi.data) {
            ...
            errorFailed(`获取${canvasTitle}失败：${createNewChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        if (updateChatflowApi.data) {
            ...
            errorFailed(`获取${canvasTitle}失败：${updateChatflowApi.error.response.data.message}`)
        }
    ...
    useEffect(() => {
        const checkIfHasChanged = async () => {
            if (getHasChatflowChangedApi.data?.hasChanged === true) {
                const confirmPayload = {
                    title: '确认变更',
                    description: `${canvasTitle} ${chatflow.name} 在你打开后已发生变化，是否覆盖？`,
    ...
    usePrompt('你有未保存的更改，确定要离开当前页面吗？', canvasDataStore.isDirty)
