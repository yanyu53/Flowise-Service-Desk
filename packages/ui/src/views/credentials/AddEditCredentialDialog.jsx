    ...
    const setOAuth2 = async () => {
        try {
            ...
            if (authResponse.data && authResponse.data.success && authResponse.data.authorizationUrl) {
                ...
                if (!authWindow) {
                    throw new Error('无法打开授权窗口，请检查浏览器是否拦截弹窗。')
                }
                ...
                const handleMessage = (event) => {
                    ...
                    if (event.data && (event.data.type === 'OAUTH2_SUCCESS' || event.data.type === 'OAUTH2_ERROR')) {
                        ...
                        if (event.data.type === 'OAUTH2_SUCCESS') {
                            ...
                            enqueueSnackbar({
                                message: event.data.message || 'OAuth2 授权失败',
                                options: {
                                    key: new Date().getTime() + Math.random(),
                                    variant: 'error',
                                    persist: true,
                ...
                throw new Error('授权接口返回无效响应')
            }
        } catch (error) {
            console.error('OAuth2 授权错误:', error)
            if (setError) setError(error)
            enqueueSnackbar({
                message: `OAuth2 授权失败：${error.response?.data?.message || error.message || '未知错误'}`,
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
        ...
        >
            ...
            <DialogContent>
                ...
                {!shared && componentCredential && componentCredential.name && componentCredential.name.includes('OAuth2') && (
                    <Box sx={{ p: 2 }}>
                        <Stack sx={{ position: 'relative' }} direction='row'>
                            <Typography variant='overline'>OAuth 回调地址</Typography>
                        </Stack>
                        <OutlinedInput
                            id='oauthRedirectUrl'
                            type='string'
