...
export const RequireAuth = ({ permission, display, children }) => {
    ...
    // if user is logged in but permission list is empty, don't hard-block all pages.
    // This prevents local instances from getting stuck on 403 due to missing RBAC payload.
    const hasEmptyPermissionPayload = Array.isArray(permissions) && permissions.length === 0
    if (hasEmptyPermissionPayload) {
        return children
    }
